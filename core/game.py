...
import core.classes
from core.classes import items
import core.func as func
from hud_elements.dialog import *
...
def main(
    ...
    player_inventory = core.classes.Inventory(app, interactables, player=True)
    ...
    player_actor = core.classes.Player(app, self_name, turret_bullets, inv = player_inventory)
    ...
    while 1:
        ...
        if app.c_weapon.jammed and click_single_tick:
            if random.uniform(0, 1) < 0.3:
                app.c_weapon.jammed = False
                gun_jam_clear.play()
        ...
        if endless:
            ...
            MInfo.trackSpeedUp = 1 + 0.01*wave_number
            MInfo.trackSpeedUp = min([MInfo.trackSpeedUp, 1.2])
        ...
        if phase != 4:
            camera_pan = app.c_weapon.__dict__["view"]
        else:
            camera_pan = 0.2
        ...
        try:
            ...
            print("EXCEPTION", e)
        fps_counter = time.time()
        core.func.keypress_manager(key_r_click, app.c_weapon, player_inventory, player_actor)

        last_camera_pos = camera_pos.copy()

        camera_pos = core.func.camera_aling(camera_pos, player_pos)
        ...
        cam_delta = core.func.minus_list(last_camera_pos, camera_pos)
        ...
        if not pvp:
            ...
            if wave:
                ...
                if map.enemy_type == "zombie":  # Kill enemies if no wave.
                    ...
                    if len(enemy_list) != 0:
                        rand_enemy = core.func.pick_random_from_list(enemy_list)
                        if not core.los.check_los(
        ...
        if (
            ...
            if player_inventory.get_amount_of_type("HE Grenade") > 0 and player_actor.preferred_nade == "HE Grenade":
                grenade_list.append(
                    armory.Grenade(
                        player_pos, core.func.minus(mouse_pos, camera_pos), "HE"
                    )
                )
                player_inventory.remove_amount("HE Grenade", 1)
                print("throwing nade")

                app.send_data(f"grenade_list.append(Grenade({player_pos}, {core.func.minus(mouse_pos, camera_pos)}, 'HE'))")

            elif player_inventory.get_amount_of_type("Molotov") > 0 and player_actor.preferred_nade == "Molotov":
                grenade_list.append(
                    armory.Grenade(
                        player_pos, core.func.minus(mouse_pos, camera_pos), "Molotov"
                    )
                )
                player_inventory.remove_amount("Molotov", 1)
                print("throwing nade")

                app.send_data(f"grenade_list.append(Grenade({player_pos}, {core.func.minus(mouse_pos, camera_pos)}, 'Molotov'))")
        ...
        if player_actor.get_hp() > 0:
            ...
            weapon_pan_rate = app.c_weapon.__dict__["handling"]

            player_angle = player_actor.get_angle()

            if abs(angle - player_angle) > 1:
                player_angle = player_angle + timedelta.mod(
                    core.los.get_angle_diff(angle, player_angle) * (weapon_pan_rate)
                )
            else:
                player_angle = angle
            ...
            if not block_movement:
                ...
                player_pos, x_vel, y_vel = core.func.player_movement2(
            ...
            for x in burn_list:
                if core.los.get_dist_points(x.pos, player_pos) < 40*multiplier2:
            ...
            if player_actor.__dict__["barricade_in_hand"] != None:
                ...
                if player_inventory.get_inv() == False and not overworld and not dialogue:
                    ...
                    firing_tick = core.func.weapon_fire(
                        app,
                        app.c_weapon,
                        player_inventory,
                    ...
                    app.c_weapon.spread_recoverial()
                    app.c_weapon.weapon_tick()
            ...
            if player_alive:
                core.func.list_play(death_sounds)
                ...
                for i in range(5):
                    particle_list.append(
                        core.classes.Particle(
                            core.func.minus(player_pos, camera_pos),
                            type="blood_particle",
                            magnitude=1.2,
                            screen=map_render,
                        )
            ...
            if not (endless and not multiplayer):
                if respawn_ticks > 0:
                    ...
                    # app.c_weapon = give_weapon(player_we[weapon_scroll])

        app.c_weapon.add_to_spread(math.sqrt(x_vel**2 + y_vel**2) / 10)
        ...
        if app.ovrdozeGT.value >= app.ovrdozeGT.max_value and app.killedThisTick:
            kill_counter = core.classes.kill_count_render(app, kill_rgb)
        ...
        if draw_los:
            ...
            draw_p_pos = core.func.minus(player_pos, camera_pos, op = "-")

            los_image, triangles, draw_time = core.render_los_image_jit.draw(
                los_image,
                phase,
        ...
        try:
            if multiplayer:
                ...
                core.func.print_s(screen, "PING: " + str(round(ping * 1000)) + "ms", 4)
        ...
        if phase != 0:
            if phase == 1:
                ...
                t = "BLOOD SINK"
                core.func.print_s(screen, "BLOODSINK: " + str(round(player_actor.bloodSink * 100)) + "%", 3)
        ...
        if player_actor.get_hp() > 0:
            ...
            if phase == 4:
                ...
                for point_dict in NAV_MESH:
                    ...
                    if core.func.get_dist_points(mo_pos_real, point) < dist:
                        dist = core.func.get_dist_points(mo_pos_real, point)
                ...
                if r_click_tick :
                    ...
                    for point_dict in NAV_MESH:
                        ...
                        if point == ref_point["point"]:
                            continue
                        if core.los.check_los(point, ref_point["point"], walls_filtered):
                ...
                if click_single_tick:
                    ...
                    route, c = core.func.calc_route(player_pos, mo_pos_real, NAV_MESH, [map.numpy_array_wall_los, map.numpy_array_wall_no_los], False, app)
                    ...
                    for x in app.path_cache:
                        ...
                        for tar in route:
                            pygame.draw.line(
                                screen,
                                [255,255,255],
                                core.func.minus(last_pos, camera_pos, op="-"),
                                core.func.minus(tar, camera_pos, op="-"),
                            )
                            last_pos = tar
                except Exception as e:
                    print(e)
                ...
                for i, x in enumerate(app.storyTeller.ammoShot):
                    core.func.print_s(screen, f"{x}:{(1-app.storyTeller.ammoShot[x])*100:.1f}%", i)

                core.func.print_s(screen, f"Perf:{(app.storyTeller.playerPerformace)*100:.1f}%", i+1)
                core.func.print_s(screen, f"{(app.enemies_within_range)} enemies", i+2)
            ...
            if dialogue != []:
                ...
                if text_str != "":
                    ...
                    for text_line in text_str[1].split("\n"):
                        ...
                        ) if not glitchy else core.func.blit_glitch(
        ...
        if phase != 5 and phase != 8:
            try:
                core.func.print_s(screen, "FPS: " + str(round(1 / (sum(fps) / 60))), 1)
                pass
            except:
                pass

            core.func.print_s(screen, "KILLS: " + str(kills), 2)
        ...
        if not app.bossTick.tick():
            ...
            if g:
                core.func.blit_glitch(screen, text, pos, 15)
            else:
                core.func.blit_glitch(screen, text, pos, 1)
        ...
        if map.name != "Downtown":
            for dropBeat in dropIndices:
                if dropBeat - 4 <= MInfo.beat_index-1 <= dropBeat:
                    ...
                    core.func.blit_glitch(screen, text, pos, round((beat_red - 1) * (5 - (dropBeat - MInfo.beat_index+1))))
        ...
        if app.screen_glitch > 0:
            image_copy = screen.copy()
            screen.fill((0,0,0))
            core.func.blit_glitch(screen, image_copy, [0,0], round(2*app.screen_glitch), black_bar_chance = 15)
