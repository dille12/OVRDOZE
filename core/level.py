...
import traceback
import core.render_los_image_jit as render_los_image_jit
from tools.image_transform import *
from utilities.quadrant import Quadrant
from tools.wall_gen import *
from core.los import check_los_jit, intersect_jit
from core.jit_tools import line_intersection
...
def load_level(map, mouse_conversion, player_inventory, app, screen, death = False):
    core.func.load_screen(app, screen, f"Loading {map.name}")
...
class Map:
    def __init__(
        ...
        self.size_converted = core.func.mult(self.size, 1 / self.conv)
        ...
        if self.CUSTOM:
            self.compiled_file = utilities.get_preferences.get_path(f"ovrdoze_data/{self.name}_compiled.cnv")

        else:

            if self.nav_mesh_name:
                name2 = self.nav_mesh_name.removesuffix(".txt")
                self.compiled_file = utilities.get_preferences.get_path(f"ovrdoze_data/{name2}_compiled.cnv")
    ...
    def getQuadrant(self, pos):
        ...
        return self.quadrants[qX][qY]

    def setToQuadrant(self, obj, pos):

        quadrant = self.getQuadrant(pos)

        if isinstance(obj, pygame.Rect):
            quadrant.walls.append(obj)

        elif obj.quadrantType == 0:
    ...
    def getQuadrantObjects(self, quadrant, type):
        ...
        for xI in range(3):
            ...
            for yI in range(3):
                ...
                if type == 3:
                    w = self.quadrants[x + xI][y + yI].rectangles
                    for xrect in w:
                        if xrect not in objects:
                            objects.append(xrect)

                if type == 4:
                    w = self.quadrants[x + xI][y + yI].block_vis_rects
                    for xrect in w:
                        if xrect not in objects:
                            objects.append(xrect)
    ...
    def generate_navmesh(self, NAV_MESH, level, loading_screen = True):
        ...
        for ref_point in NAV_MESH:
            ...
            for point_dict in NAV_MESH:
                ...
                if tolerance:
                    for x, y in ([tolerance,tolerance], [-tolerance,tolerance], [-tolerance,-tolerance], [tolerance, -tolerance]):
                        for x2, y2 in ([tolerance,tolerance], [-tolerance,tolerance], [-tolerance,-tolerance], [tolerance, -tolerance]):
                            if not core.los.check_los_jit(np.array([point[0]+x, point[1]+y]), np.array([ref_point["point"][0]+x2, ref_point["point"][1]+y2]), self.numpy_array_wall_los, self.numpy_array_wall_no_los):
                                good_point = False
                                break
                    if not good_point:
                        continue
                ...
                if core.los.check_los_jit(np.array(point), np.array(ref_point["point"]), level.numpy_array_wall_los, level.numpy_array_wall_no_los):

                    good_point = True
                    for point2 in ref_point["connected"]:

                        angle_to_point = core.func.get_angle(ref_point["point"], point2)
                        angle_to_point2 = core.func.get_angle(ref_point["point"], point)

                        if abs(core.los.get_angle_diff(angle_to_point2, angle_to_point)) < 30:
                            good_point = False

                            ref_point["connected"].remove(point2)

                            if core.func.get_dist_points(ref_point["point"], point2) < core.func.get_dist_points(ref_point["point"], point):
    ...
    def checkcollision(
        ...
        QUADRANT = self.getQuadrant(pos)
        RECTS = self.getQuadrantObjects(QUADRANT, 3) if not bullet else self.getQuadrantObjects(QUADRANT, 4)

        for check in check_order:

            collisions = list(getcollisions(RECTS, collider))

            if check == check_order[0] and damage_barricades:
                for barr in self.barricade_rects:
                    if barr[0] in collisions:
                        barr[1].hp -= damager.damage
                        damager.attack_tick = 30
    ...
    def generate_wall_structure2(self):
        ...
        for polygon in self.polygons:
            ...
            for x in self.quadrants:
                for y in x:
                    if poly.colliderect(y.rect):
                        y.rectangles.append(poly)
                        y.block_vis_rects.append(poly)
                        print("QUADRANT:", y.x, y.y)
        ...
        for polygon in self.polygons_no_los_block:
            ...
            self.no_los_walls.append(core.los.Wall(a, b, pol=polygon))
            self.no_los_walls.append(core.los.Wall(b, c, pol=polygon))
            self.no_los_walls.append(core.los.Wall(c, d, pol=polygon))
            self.no_los_walls.append(core.los.Wall(d, a, pol=polygon))
        ...
        for polygon in self.polygons:
            ...
            # c = ratio(c,size_ratio)
            # d = ratio(d,size_ratio)
            walls.append(core.los.Wall(a, b, pol=polygon))
            walls.append(core.los.Wall(b, c, pol=polygon))
            walls.append(core.los.Wall(c, d, pol=polygon))
            walls.append(core.los.Wall(d, a, pol=polygon))
        ...
        for wall1 in walls:
            ...
            for wall2 in walls:
                ...
                if core.func.get_dist_points(p1, p3) < 3:
                    wall2.set_new_points(p1, p4)
                if core.func.get_dist_points(p2, p3) < 3:
        ...
        for wall_1 in walls:
            ...
            for wall_2 in walls:
                ...
                if p1 in wall_points or p2 in wall_points:
                    continue
                if core.los.intersect(
                    wall_points[0],
                    wall_points[1],
                    core.func.minus(p1, [-3, -3]),
                    core.func.minus(p2, [3, 3]),
                ):
        ...
        for i in range(3):
            ...
            for wall1 in walls:
                ...
                if core.func.get_dist_points(p1, p2) < 3:
                ...
                for wall2 in walls:
                    ...
                    if (
                        core.los.intersect(
                            core.func.minus(p1, [2, 2]),
                            core.func.minus(p2, [-2, -2]),
                            core.func.minus(p3, [2, 2]),
                            core.func.minus(p4, [-2, -2]),
                        )
            ...
            for wall_2 in walls:
                ...
                for point in wall_1_points:
                    for point2 in wall_2.get_points():
                        if core.los.get_dist_points(point, point2) < 5:
                            interlink1 = point
                            interlink2 = point2
                ...
                if interlink1 != None and interlink2 != None:
                    ...
                    if core.los.intersect(
                        wall_points[0],
                        wall_points_2[0],
                        core.func.minus(interlink1, [-3, -3]),
                        core.func.minus(interlink1, [3, 3]),
                    ):
    ...
    def generate_wall_structure(self):
        ...
        for polygon in self.polygons:
            ...
            # c = ratio(c,size_ratio)
            # d = ratio(d,size_ratio)
            walls.append(core.los.Wall(a, b))
            walls.append(core.los.Wall(b, c))
            walls.append(core.los.Wall(c, d))
            walls.append(core.los.Wall(d, a))
        ...
        for polygon in self.polygons:
            ...
            if poly.collidepoint(player_pos_der):
                ...
                if (
                    ...
                    player_pos_der[0] = core.func.get_closest_value(
                ...
                if (
                    ...
                    player_pos_der[1] = core.func.get_closest_value(
    ...
    def getPointBasedOnBlood(self, p_pos, max_tries = 13):
        ...
        while tries < max_tries:
            point = core.func.pick_random_from_list(self.nav_mesh_available_spots)
            visible = core.los.check_los_jit(np.array(p_pos), np.array(point), self.numpy_array_wall_los)
    ...
    def getFurthest(self, pos, l):
        ...
        furthest_p = None
        for x in l:
            d = core.los.get_dist_points(pos, x)
            if d > furthest:
                furthest_p = x
                furthest = d
    ...
    def get_random_point(
        ...
        furthest_p = core.func.pick_random_from_list(self.nav_mesh_available_spots)
        furthest = 0
        while tries < max_tries:
            tries += 1
            point = core.func.pick_random_from_list(self.nav_mesh_available_spots)
            ...
            if p_pos != None:
                if core.los.check_los_jit(np.array(p_pos), np.array(point), walls):
                    conds[0] = False
            if enemies != None:
                for x in enemies:
                    if core.los.check_los_jit(np.array(point), np.array(x.get_pos()), walls):
                        conds[1] = False
                        break
            if visible_from_origin_point:
                if not core.los.check_los_jit(np.array(point), np.array(visible_from_origin_point), walls):
                    conds[2] = False

            if max_dist:
                if core.los.get_dist_points(point, max_dist_point) > max_dist:
                    conds[3] = False

            if furthest_point_from_point:
                if False not in conds:
                    dist = core.los.get_dist_points(point, furthest_point_from_point)
                    if dist > furthest:
                        furthest = dist
                        furthest_p = point
                if tries > max_tries:
