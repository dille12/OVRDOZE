...
#@nb.njit
def finish_intersects(start_pos,points, wall_points):
    visible_points = points.copy()
    i = 0
    for point_array in points:
        point_array2 = point_array.astype(np.int64)
        angle = point_array[0]
        point = point_array[0:2]
        intersecting_ray = False
        for wall_1 in wall_points:
            is_wall_point = False
            for wall_point in wall_1:
                if np.array_equal(wall_point, point):
                    is_wall_point = True
                    print("IS WALL POINT")
                    break

            print(wall_1[0], wall_1[1])

            if not is_wall_point and intersect(nb.typed.List(start_pos), nb.typed.List(point), nb.typed.List(wall_1[0]), nb.typed.List(wall_1[1])):
                print("Intersects")
        ...
        if intersecting_ray:
            visible_points[i] = [0,0,0]
            #visible_points.append(point_array)
...
for i in range(6):
    t = time.time()
    points = np.array([[90,1,4], [180,2,0], [20,1,1], [60,3,2]])
    wall_points = np.array([np.array([[0,2], [3,2]])])
    visible_points = finish_intersects(np.array([1,1]), points, wall_points)
    #print(visible_points)
    print(time.time() - t)
print(visible_points)

print(intersect([1,1], [1,4], [0,2], [3,2]))

print(intersect(nb.typed.List(start_pos), nb.typed.List(point), nb.typed.List(wall_1[0]), nb.typed.List(wall_1[1])))
