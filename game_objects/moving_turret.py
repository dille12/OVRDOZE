...
class MovingTurret(Game_Object):
    ...
    def draw_bead_on(self,aim_at,shoot):
        ...
        turret2, turret_rect = func.rot_center(mov_turret_gun,self._angle-90,self._pos[0],self._pos[1])
    ...
    def draw(self, screen,camera_pos,turret2,turret_rect):
        ...
        turret_base, turret_base_rect = func.rot_center(mov_turret_base,self.base_angle-90,self._pos[0],self._pos[1])


        screen.blit(turret_base, func.draw_pos(turret_base_rect,camera_pos)) #func.minus_list, [self.size, self.size])
    ...
    def get_route_to_target(self, target):
        if self.route_tick == 0:
            self.route_tick = 60
            self.route = func.calc_route(self._pos, target, self.navmesh_ref, self.wall_ref, quick = False)
