...
def thread_data_collect(
    net, packet, multiplayer_actors, bullet_list, grenade_list, current_threading
):
    try:
        ...
        bullet_list, grenade_list = network_parser.gen_from_packet(
            reply, multiplayer_actors, bullet_list, grenade_list
        )
...
def main(
    ...
    diff_rates = {
        ...
        "SUPER EASY": [0.1, 0.9, 0.75, 1, 1.2],
        "EASY": [0, 0.9, 0.75, 1, 1.2],
        "NORMAL": [1, 1, 1, 1, 6],
        "HARD": [1.25, 1.25, 1.1, 0.85, 10],
        "ONSLAUGHT": [1.5, 1.35, 1.2, 0.7, 14],
    }  #
    ...
    for map_1 in active_maps:
        ...
        for i in range(2):
            end_point = (
                map_1.__dict__["pos"][i] * map_conversion + map_1.__dict__["size"][i]
            ) / map_conversion
    ...
    while 1:
                ...
                if False:  # Kill enemies if no wave.
                    ...
                    if len(enemy_list) != 0:
                        if random.uniform(0, 1) < 0.1:
                            func.pick_random_from_list(enemy_list).kill(
                                camera_pos, enemy_list, map_render, silent=True
                            )

                if time.time() - wave_change_timer > wave_interval and wave_number < 8:
                    wave_length += 3
                    # wave_interval += 1
                    wave = True
                    wave_number += 1
        ...
        if multiplayer:
            ...
            for x in multiplayer_actors:
                multiplayer_actors[x].tick(
                    screen, player_pos, camera_pos, walls_filtered
                )
        ...
        if player_actor.get_hp() > 0:
            ...
            if player_actor.__dict__["barricade_in_hand"] != None:
                ...
                if player_actor.__dict__["barricade_in_hand"].tick(
        ...
        if draw_los:
            los_image, draw_time = los.render_los_image(
                los_image,
                phase,
                camera_pos,
                player_pos,
